function importFile(fileInput) {
    ...
    if (file.type.match(ALLOWED_FILETYPE)) {
        ...
        reader.onload = function(e) {
            ...
            // allpapers = analyzePaperOnServer(document_text);
            allpapers = createDummyPapers();
            return allpapers;
        }
    } else {
...
function createDummyPapers() {
    ...
    var allpapers = [paper,paper,paper,paper]
    displayPapers(allpapers)
}

function analyzePaperOnServer(document_text) {
    var SERVER_ENDPOINT = "http://localhost:5000/upload";
    $.post(SERVER_ENDPOINT, 
            input = JSON.stringify(document_text), 
            success=function( response ) {
                allpapers = JSON.parse(response);
                return allpapers;
            }); 
}