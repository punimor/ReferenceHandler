...
function display_papers(allpapers){
    var i;
    for (i in allpapers) {
        var markup = "<tr class='unselected' data-toggle='collapse' data-target='.collapsingItem" + String(i) + "'><td id ='table-rating'>" + "<div class='rateYo" + String(i) + "'></div>" + "</td><td><h5>" + allpapers[i]['title'] + "</h5><div class='collapse collapsingItem" + String(i) + "'>" + allpapers[i]['abstract'] + "</div></td><td><button type='button' class='btn btn-outline-secondary btn-block undecide decisionbutt'></button><div class='collapse collapsingItem" + String(i) + "'><button type='button' class='btn btn-outline-success btn-block button-adjust include'></button><button type='button' class='btn btn-outline-info btn-block button-adjust maybe'></button><button type='button' class='btn btn-outline-danger btn-block button-adjust exclude'></button></div></td></tr>"
    ...
    $(".rateYo0").rateYo({
    rating: Math.round(parseFloat(allpapers[i]['rating']) * 10) / 10
    });

    // Setter
    $(".rateYo0").rateYo("option", "starWidth", "20px"); //returns a jQuery Element
    });
